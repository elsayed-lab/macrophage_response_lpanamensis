...
```{r}
type_batch <- set_batches(hs_macrophage, fact = "typeofcells")
type_batch_norm <- normalize(type_batch, norm = "quant", transform = "log2",
...
```{r}
hs_macr <- subset_se(hs_macrophage, subset = "typeofcells=='Macrophages'")

written <- write_se(
  hs_macr,
  excel = glue("analyses/macrophage_de/{ver}/read_counts/hs_macr-v{ver}.xlsx"))

macr_norm <- normalize(hs_macr, norm = "quant", transform = "log2",
                       convert = "cpm", filter = TRUE)
...
hs_macr_datebatch <- set_batches(hs_macr, fact = "oldnew")
macr_date_norm <- normalize(hs_macr_datebatch, norm = "quant", transform = "log2",
                            convert = "cpm", filter = TRUE)
...
```{r}
hs_nb <- normalize(hs_macr, convert = "cpm", transform = "log2",
...
hs_macr <- set_conditions(hs_macrophage, fact = new_conditions) %>%
  ...
  set_colors(color_choices[["treatment_zymo"]])
```
...
```{r}
hs_macr_norm <- normalize(hs_macr, filter = TRUE, convert = "cpm",
...
```{r}
hs_macr_nb <- normalize(hs_macr, filter = TRUE, convert = "cpm",
...
hs_macr_nb_noshape <- set_batches(hs_macr_nb, fact = "experiment")
...
table(colData(hs_macr)[["drug"]])
table(colData(hs_macr)[["macrophagezymodeme"]])
table(colData(hs_macr)[["rnaextractiondate"]])
table(colData(hs_macr)[["oldnew"]])

donor_drug_zymo_varpart <- simple_varpart(
  hs_macr, fstring = "~ donor + drug + macrophagezymodeme")
...
time_drug_zymo_varpart <- simple_varpart(
  hs_macr, fstring = "~ oldnew + drug + macrophagezymodeme")
...
```{r}
drug_donor_test <- set_conditions(hs_macr, fact = "drug") %>%
  set_batches(fact = "donor")
drug_donor_norm <- normalize(drug_donor_test, norm = "quant", filter = TRUE,
...
```{r}
drug_infectedp_test <- set_conditions(hs_macr, fact = "drug") %>%
  set_batches(fact = "batch") %>%
  set_colors(color_choices[["drug"]])
drug_infectedp_norm <- normalize(drug_infectedp_test, norm = "quant", filter = TRUE,
                                 convert = "cpm", transform = "log2")
...
drug_zymo_donor_varpart <- simple_varpart(
  hs_macr, fstring = "~ drug + macrophagezymodeme + donor")
...
```{r}
combined_batch <- paste0(colData(hs_macr)[["drug"]], "_",
                         colData(hs_macr)[["macrophagezymodeme"]])
hs_donors <- set_conditions(hs_macr, fact = "donor") %>%
  set_batches(fact = "macrophagezymodeme") %>%
  set_colors(color_choices[["donor"]])

donor_norm <- normalize(hs_donors, filter = TRUE,
                        convert = "cpm", transform = "log2")
...
donor_nb <- normalize(hs_donors, filter = TRUE, norm = "quant",
...
```{r}
hs_drug <- set_conditions(hs_macr, fact = "drug") %>%
  set_colors(color_choices[["drug"]])

drug_norm <- normalize(hs_drug, filter = TRUE, norm = "quant",
                       convert = "cpm", transform = "log2")
...
drug_nb <- normalize(hs_drug, filter = TRUE, norm = "quant",
                     convert = "cpm", transform = "log2", batch = "svaseq")
...
```{r}
lp_norm <- normalize(lp_macrophage, norm = "quant", transform = "log2",
                     convert = "cpm", filter = TRUE)
...
lp_nb <- normalize(lp_macrophage, convert = "cpm", transform = "log2",
...
```{r}
lp_norm_nosb <- normalize(lp_macrophage_nosb, norm = "quant", transform = "log2",
                          convert = "cpm", filter = TRUE)
...
lp_nb_nosb <- normalize(lp_macrophage_nosb, convert = "cpm", transform = "log2",
                        filter = TRUE, batch = "svaseq")
...
wanted %in% rowData(lp_macrophage)[["annot_gene_name"]]
ids <- rowData(lp_macrophage)[["annot_gene_name"]] %in% wanted
ids <- rownames(assay(lp_macrophage))[ids]
names <- rowData(lp_macrophage)[ids, "annot_gene_name"]
lp_norm <- normalize(lp_macrophage, transform = "log2", convert = "cpm", norm = "quant")
